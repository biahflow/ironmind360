...
- Status geral: **Fase 4 concluída**; **Fase 5 — Blocos 1 (Infraestrutura ML), 2 (Carga/overtraining) e 3 (Anomalias) concluídos**.
- Fase atual: **Fase 5 — ML preditivo**. Próximo bloco: **Previsão de performance em prova** (Gradient Boosting, `POST /ml/race-prediction`).
- Serviço `ml/` no ar (FastAPI/8100): features, risco de overtraining, detecção de anomalias (Isolation Forest por tipo de atividade), versionamento, cache Redis; proxy `/api/v1/ml/*` com `/status`, `/retrain`, `/overtraining-risk`, `/anomalies`; risco integrado à readiness; anomalies e overtraining no dashboard/home.
