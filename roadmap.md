...
- Status geral: **Fase 4 concluída**; **Fase 5 — Blocos 1 (Infraestrutura ML) e 2 (Carga/overtraining) concluídos**.
- Fase atual: **Fase 5 — ML preditivo**. Próximo bloco: **Detecção de anomalias em sessões** (Isolation Forest sobre o perfil estatístico do atleta; `POST /ml/anomalies`).
- Serviço `ml/` no ar (FastAPI/8100): features (ACWR/monotonia/strain, sono, RPE), modelo composto de risco de overtraining, versionamento, cache Redis; proxy `/api/v1/ml/*` com `/status`, `/retrain`, `/overtraining-risk`; risco integrado à readiness e ao card da home.
